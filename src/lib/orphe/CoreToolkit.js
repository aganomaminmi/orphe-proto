...
export var bles = [new Orphe(0), new Orphe(1)];
...
export function buildCoreToolkit(
  ...
  let div_form_check = CTbuildElement(
    "div",
    "",
    "form-ckeck form-switch d-flex",
    "",
    parent_element
  );
  ...
  let span_activity = CTbuildElement(
    "span",
  ...
  let span_battery = CTbuildElement(
    "span",
  ...
  let span_led = CTbuildElement(
    "span",
    `<i class="bi bi-brightness-alt-high position-relative"><span class="position-absolute  top-0 start-50 tanslate-middle badge text-muted bg-light" style="font-size:0.2em;" id="led_number${core_id}">
          0
        </span>
      </i>`,
    "text-muted ms-1",
  ...
  let span_settings = CTbuildElement(
    "span",
    `<i
        class="bi bi-gear"></i>`,
    "text-muted ms-1",
  ...
  span_settings.setAttribute(
    "title",
    `settings for notification, sensor ranges.`
  );
  ...
  let div_modal_dialog = CTbuildElement(
    "div",
    "",
    "modal-dialog text-dark",
    "",
    div_modal
  );
  let div_modal_content = CTbuildElement(
    "div",
    "",
    "modal-content",
    "",
  ...
  let div_modal_header = CTbuildElement(
    "div",
    `<h5 class="modal-title" id="exampleModalLabel"><i class="bi bi-gear"></i> CORE0${core_id} Settings</h5 >
  ...
  let div_modal_body = CTbuildElement(
    "div",
  ...
  else if (notification == "SENSOR_VALUES")
    select_notify.options[1].selected = true;
  else if (notification == "STEP_ANALYSIS_AND_SENSOR_VALURS")
    select_notify.options[2].selected = true;
...
async function toggleCoreModule(dom, options = {}) {
  ...
  if (checked == true) {
    ...
    ble.gotBLEFrequency = function (freq) {
      document.querySelector(`#freq${this.id} `).innerHTML =
        `${Math.floor(freq)} Hz`;
    };
  } else {
    ble.reset();
...
export function changeNotify(no, dom) {
...
export function resetCoreModule(id) {
...
async function updateBatteryInfo(dom) {
  ...
  document
    .querySelector(`#icon_battery${number} `)
    .setAttribute("title", `${str_battery_status} `);

  if (obj.battery == "0") {
    document.querySelector(`#icon_battery${number} `).innerHTML =
      '<i class="bi bi-battery"></i>';
    document.querySelector(`#icon_battery${number} `).classList =
      "text-warning";
...
export function setHeaderStatusOffline(id) {
  if (typeof document === "undefined") return;
  document.querySelector(`#switch_ble${id} `).checked = false;
}

export function CTbuildElement(
  name_tag,
  innerHTML,
  str_class,
  str_style,
